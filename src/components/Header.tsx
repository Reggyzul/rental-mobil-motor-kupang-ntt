    ...
    <header className="fixed top-0 left-0 w-full z-50 transition-all duration-300">
      ...
      <div
        className={`w-full transition-all duration-300 ${
          isScrolled
            ? 'bg-white/95 backdrop-blur-md py-2.5 shadow-sm border-b border-slate-100'
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-4">
            ...
            >
              ...
              <div className="flex flex-col leading-tight">
                <span className="font-display font-black text-base sm:text-lg tracking-tight text-[#081836] uppercase">
                  Rizal Transportasi <span className="text-sky-600">Batam</span>
                </span>
            ...
            <nav className="hidden lg:flex items-center justify-center gap-8 text-xs uppercase tracking-wider font-sans font-extrabold text-slate-700 flex-1 px-4" id="desktop-nav">
              {navItems.map((item) => {
            ...
            <div className="hidden lg:flex items-center gap-3 shrink-0">
              ...
              <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200/80 text-xs font-bold">
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg transition-all cursor-pointer flex items-center gap-1 ${
                    lang === 'ID'
                      ? 'bg-[#081836] text-white shadow-xs font-extrabold'
              ...
              <button
                ...
                className="bg-gradient-to-r from-[#081836] via-[#0e2a5c] to-sky-600 hover:from-[#0a2048] hover:to-sky-500 text-white font-display font-black text-xs uppercase px-5 py-2.5 rounded-xl shadow-md hover:shadow-sky-500/20 transition-all duration-300 transform hover:-translate-y-0.5 cursor-pointer tracking-wider flex items-center gap-1.5"
                id="header-book-btn"
              >
            ...
            <div className="px-5 pt-3 pb-6 space-y-2">
                ...
                return (
                  <button
                    ...
                    className={`block w-full text-left font-display font-bold text-sm py-2.5 border-b border-slate-100 transition-colors ${
                      isActive ? 'text-sky-600 font-extrabold' : 'text-slate-800 hover:text-sky-600'
                    }`}
                  >
                    {item.label}
                  </button>
                ...
                <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-bold">
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg transition-all cursor-pointer ${
                      lang === 'ID'
                        ? 'bg-[#081836] text-white font-extrabold'
                        : 'text-slate-600'
                    }`}
                  >
                    🇮🇩 ID
              ...
              <div className="pt-3">
                <button
                  ...
                  className="w-full bg-gradient-to-r from-[#081836] to-sky-600 text-white font-display font-bold text-xs uppercase py-3.5 rounded-xl shadow-md flex items-center justify-center gap-2"
                >
